...
class App extends Component {
  ...
  componentDidMount() {
    this.timerId = setInterval(this.status, 1000)
  }
  ...
  reqImg = () => {
    ...
    const filterData = imagesList.filter(each => each.category === active)
    return filterData
  }

  bigImg = thumbnailUrl => {
    const {imgUrl} = this.state
    const image = imagesList.filter(each => each.thumbnailUrl === thumbnailUrl)
  ...
  render() {
    const {imgUrl, score, time, isTrue, active} = this.state

    const filterList = this.reqImg()
    return (
      <div>
        <nav className="navCont">
          ...
          />
          <ul className="navSec">
            <li className="navPara1">
              <p>
                Score:<span className="spans"> {score}</span>
              </p>
            </li>
            <li>
              <img
            ...
            <p className="navPara2">{time} sec</p>
          </ul>
        </nav>
        {isTrue && (
          <div className="cardCont1">
            ...
            <ul className="unis">
              {tabsList.map(each => (
                <li className="lis" key={each.tabId}>
                  <button
                    type="button"
                    className={`tabButton ${
                      active === each.tabId ? 'border-line' : ''
                    }`}
                    onClick={() => this.tabImages(each.tabId)}
                  >
                    {each.displayText}
                  </button>
                </li>
            ...
            <ul className="unis2">
              {filterList.map(each => (
                <li className="lis" key={each.id}>
                  <button
                    type="button"
                    className="imgButton"
                    onClick={() => this.bigImg(each.thumbnailUrl)}
                  >
                    <img
          ...
          <div className="cardCont2">
            <div className="card">
              <img
                ...
                className="trophy"
              />
              <p className="scoreHead">YOUR SCORE</p>
              <p className="scoreHead2">{score}</p>
